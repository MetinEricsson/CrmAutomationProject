...
public class SalePage extends MainPage {
    ...
    private static By sonrakiAdim_MusteriSecimi_Btn= By.xpath("//button[@type='submit'][text()='Sonraki Adım']");
    private static By sec_Btn=By.xpath("//button[text()='Seç']");
    ...
    public SalePage fiberPaketSec(int item){
    ...
    public SalePage urunKontrol_GozdenGecir(int numberOfChosenProduct){
        ...
        try {
            ...
            waitElementWithThreadSleepViaItem(secimiIptalEt_GozdenGecir_Btn,5);

            int getChosenProductItem= getSizeOfList(secimiIptalEt_GozdenGecir_Btn);

            if (getChosenProductItem==numberOfChosenProduct){
                reporter.Report_Info("Secilen paketler ekran uzerine dogru yansimistir.");
            }else{
                reporter.Report_Fail("Secilen urunler ekrana eksik yansimistir!");
            }
